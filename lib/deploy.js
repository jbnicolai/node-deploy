...
var execute = function(server, port, commands, callback) {
  exec('ssh -A -p ' + port + ' ' + server + ' "' + commands + '"', function(err, output) {
...
var run = function(settings) {
  var verify = function() {
    execute(settings.server, settings.sshport, createVerifyScript(settings));
  };

  execute(settings.server, settings.sshport, createDeployScript(settings), function() {
...
var remove = function(settings) {
  execute(settings.server, settings.sshport, createRemoveScript(settings));
};
