...
var generate = function(callback) {
  var askQuestions = function(options, callback) {
    ...
    var allQuestionsAnswered = function(err, answers) {
      ...
      return callback({
        ...
        start:   answers[2] ? answers[2] : options.start,
        ...
        server:  getServerAddress(answers[7]),
        sshport: getSSHPort(answers[7])
      });
    };
    ...
    async.series([
      ...
      function(cb) {
        io.question('app start command' + wrapDefault(options.start), success(cb));
      },
      function(cb) {
  ...
  defaults.find(function(options) {
    return askQuestions(options, callback);
  });
};
...
var validate = function(config) {
  ...
  if (!config.start)  error('No app start command was specified');
  ...
  if (ok && config.sshport && !parseInt(config.sshport)) error('ssh port must be a valid port number');
